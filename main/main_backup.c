...
int main(void) {
	...
	while(1){
		...
		/*
		//if up or down button
		int32_t pinread = GPIOPinRead(GPIO_PORTE_BASE, GPIO_PIN_5);
		if(pinread==0) buttonPress_output0(); // down
		pinread = GPIOPinRead(GPIO_PORTN_BASE, GPIO_PIN_3);
		if(pinread==0) buttonPress_output1(); // up


		//reset air
		pinread = GPIOPinRead(GPIO_PORTP_BASE, GPIO_PIN_1);
		if(pinread==0) {
